 ...
 */
ctrlrs.controller('errorDialogCtrl',['$scope','$modalInstance','$translate', '$timeout', 'data',function($scope,$modalInstance,$translate,$timeout,data){
	//-- Variables -----//
	...
	if (angular.isDefined(data.timer)) {
		$timeout(function() {
			$scope.close();
		}, data.timer);
	}
	...
	.provider('dialogs',[function(){
		...
		this.$get = ['$modal',function ($modal){
			...
			return {
				 ...
				 * @param	opts	object
				 */
				error : function(header,msg,timer,opts){
					opts = _setOpts(opts);
